...
int syn_scan(struct s_scan *scan, struct s_port *ports, struct timeval timeout)
{
	int tcpsockfd;
	int icmpsockfd;
	int ret;
	...
	if (send_syn(tcpsockfd, &scan->saddr, &scan->daddr) != 0) {
		...
		UNLOCK(scan);
		while (!(ret = read_syn_ack(tcpsockfd, icmpsockfd, scan,
			timeout, ports)));
		...
		if (ret == TIMEOUT) {
			...
			if (send_syn(tcpsockfd, &scan->saddr, &scan->daddr) != 0) {
				...
				/* Successful send */
				UNLOCK(scan);
				while (!(ret = read_syn_ack(tcpsockfd, icmpsockfd,
					scan, timeout, ports)));
