...
static int send_ack(int tcpsockfd,
	struct sockaddr_in *saddr, struct sockaddr_in *daddr)
{
	...
	craft_tcp_packet(packet, saddr, daddr, TH_ACK, NULL);

	/* Verbose print */
	if (g_data.opt & OPT_VERBOSE_INFO || g_data.opt & OPT_VERBOSE_DEBUG)
		fprintf(stderr, "[*] Sending ACK request to: %s:%d from port %d\n",
	...
	if (sendto(tcpsockfd, packet, sizeof(packet), 0, (struct sockaddr *)daddr,
		...
		if (g_data.opt & OPT_VERBOSE_INFO || g_data.opt & OPT_VERBOSE_DEBUG)
			fprintf(stderr, "[!] Failed to send ACK packet to: %s:%d from port %d\n",
			inet_ntoa(daddr->sin_addr), ntohs(daddr->sin_port),
			ntohs(saddr->sin_port));
		return 1;
	}
	...
	struct timeval timeout)
{
	...
	if (status != -1) {
		 ...
		 * requests */
		if ((update_ret = update_scans(scan, status, dest, OPT_SCAN_ACK))) {
			if ((g_data.opt & OPT_VERBOSE_INFO || g_data.opt & OPT_VERBOSE_DEBUG))
			{
...
int ack_scan(struct s_scan *scan)
{
	...
	if (send_ack(tcpsockfd, scan->saddr, scan->daddr) != 0) {
		...
		if (ret == TIMEOUT) {
			LOCK(scan);
			if (g_data.opt & OPT_VERBOSE_INFO || g_data.opt & OPT_VERBOSE_DEBUG)
				fprintf(stderr, "[*] ACK request on %s:%d timedout\n",
	...
	if (g_data.opt & OPT_VERBOSE_INFO || g_data.opt & OPT_VERBOSE_DEBUG) {
		fprintf(stderr, "[*] Updating %s:%d ACK scan to %s\n",
		inet_ntoa(scan->daddr->sin_addr), ntohs(scan->daddr->sin_port),
		status[scan->status]);
	}
